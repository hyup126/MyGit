package kr.or.ddit.admin.answer.service.impl;
...
import kr.or.ddit.admin.answer.controller.AdminAnswerUploadUtils;
import kr.or.ddit.admin.answer.mapper.AdminIAnswerMapper;
...
@Slf4j
@Service
public class AdminAnswerServiceImpl implements AdminIAnswerService {
	
	@Inject
	private AdminIAnswerMapper mapper;
	...
	public ServiceResult insertAnswer(HttpServletRequest req, AnswerVO answerVO) throws Exception {
	    ...
	    if (status > 0) {
	        ...
	        AdminAnswerUploadUtils.ansFileUpload(answerFileList, answerVO.getAnsNo(), req, mapper);
	        result = ServiceResult.OK;
	    } else {
	        result = ServiceResult.FAILED;
	    }
	    ...
	    return result;
			...
			public List<AnswerCommentVO> selectCommentList(int ansNo) {
				...
				return mapper.selectCommentList(ansNo);
			}
			@Override
			...
			public int deleteComment(AnswerCommentVO commentVO) {
				return mapper.deleteComment(commentVO);
			}
